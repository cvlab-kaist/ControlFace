...
def prepare_image_encoder():
    ...
    os.makedirs(local_dir, exist_ok=True)
    for hub_file in ["image_encoder/config.json", "image_encoder/pytorch_model.bin", "unet/config.json", "unet/diffusion_pytorch_model.bin"]:
        path = Path(hub_file)
        saved_path = local_dir / path
        if os.path.exists(saved_path):
            continue
...
def prepare_control():
    print(f"Preparing ControlFace weights...")
    local_dir = "./pretrained_weights/ControlFace"
    os.makedirs(local_dir, exist_ok=True)
    for hub_file in [
        "denoising_unet.pth",
        "point_embedding.pth",
